...
const msSubRoot = document.getElementById('msSub');
const msUserRoot = document.getElementById('msUser');
...
function groupRows(rows){
  ...
  if(hasEvent){
    for(let i=0;i<data.length;){
      ...
      const assembled = { style:null, category:null, output:null, ts:null, country:null, subStatus:null, userPseudoId:null };
      for(const r of chunk){
        ...
        if(assembled.subStatus == null && idxKey.subStatus !== -1){
          const sv = r[idxKey.subStatus];
          if(sv && sv !== '#N/A') assembled.subStatus = sv;
        }
        if(assembled.userPseudoId == null && idxKey.userPseudoId !== -1){
          const up = r[idxKey.userPseudoId];
          if(up) assembled.userPseudoId = up;
        }
      }
      if(assembled.output){
        records.push({
  ...
  let acc = {style:null, category:null, output:null, ts:null, country:null, subStatus:null, userPseudoId:null};
  function flush(){
    ...
    records.push({
      ...
      timestamp: acc.ts || '',
      country: acc.country || '',
      subscriptionStatus: acc.subStatus || '',
      userPseudoId: acc.userPseudoId || ''
    });
    acc = {style:null, category:null, output:null, ts:null, country:null, subStatus:null, userPseudoId:null};
  }
  for(const r of data){
    ...
    if(key === 'style'){
      ...
      acc.style = val; acc.ts = ts; acc.country = country; acc.subStatus = subStatus; acc.userPseudoId = idxKey.userPseudoId !== -1 ? r[idxKey.userPseudoId] : acc.userPseudoId;
...
let msCountry = null, msCategory = null, msStyle = null, msSub = null, msUser = null;
...
function populateFilters(records){
  ...
  msSub ||= createMultiSelect(msSubRoot, 'Subscription');
  msUser ||= createMultiSelect(msUserRoot, 'User');
  ...
  msSub?.setData(countMap(records.map(r=>r.subscriptionStatus||'')));
  msUser?.setData(countMap(records.map(r=>r.userPseudoId||'')));
  ...
  if(msSub) msSub.onChange(()=>{ currentPage = 1; render(); });
  if(msUser) msUser.onChange(()=>{ currentPage = 1; render(); });
}
...
function recordMatchesFilters(r){
  ...
  const subs = msSub ? new Set(msSub.getSelected()) : null;
  const users = msUser ? new Set(msUser.getSelected()) : null;
  ...
  if(subs && subs.size && !subs.has(r.subscriptionStatus||'')) return false;
  if(users && users.size && !users.has(r.userPseudoId||'')) return false;
  return true;
}

// Facet counts that respond to other selections
function updateFacetCounts(){
  const sets = {
    country: msCountry ? new Set(msCountry.getSelected()) : new Set(),
    categoryName: msCategory ? new Set(msCategory.getSelected()) : new Set(),
    styleName: msStyle ? new Set(msStyle.getSelected()) : new Set(),
    subscriptionStatus: msSub ? new Set(msSub.getSelected()) : new Set(),
    userPseudoId: msUser ? new Set(msUser.getSelected()) : new Set()
  };
  const keys = Object.keys(sets);
  function matchesExcept(rec, skipKey){
    for(const k of keys){
      if(k === skipKey) continue;
      const sel = sets[k];
      if(sel && sel.size){
        const v = (rec[k] || rec[k] === '' ? rec[k] : (rec[k] ?? ''));
        if(!sel.has(v||'')) return false;
      }
    }
    return true;
  }
  // Compute counts per facet with other filters applied
  if(msCountry){
    const data = countMap(allRecords.filter(r=>matchesExcept(r,'country')).map(r=>r.country||''));
    msCountry.setData(data);
  }
  if(msCategory){
    const data = countMap(allRecords.filter(r=>matchesExcept(r,'categoryName')).map(r=>r.categoryName||''));
    msCategory.setData(data);
  }
  if(msStyle){
    const data = countMap(allRecords.filter(r=>matchesExcept(r,'styleName')).map(r=>r.styleName||''));
    msStyle.setData(data);
  }
  if(msSub){
    const data = countMap(allRecords.filter(r=>matchesExcept(r,'subscriptionStatus')).map(r=>r.subscriptionStatus||''));
    msSub.setData(data);
  }
  if(msUser){
    const data = countMap(allRecords.filter(r=>matchesExcept(r,'userPseudoId')).map(r=>r.userPseudoId||''));
    msUser.setData(data);
  }
}
...
function render(){
  // Update facet counts first so they reflect current selections
  updateFacetCounts();
  ...
  stripEl.innerHTML = stripItems.map((r,i)=>{
    const cancelled = (r.subscriptionStatus||'').toLowerCase().includes('cancelled_by_user');
    const tag = cancelled ? '<span class="tag danger">subscription_cancelled_by_user</span>' : (r.subscriptionStatus ? `<span class="tag">${escapeHtml(r.subscriptionStatus)}</span>` : '');
    return `
      <div class="chip${cancelled?' cancelled':''}" data-index="${i}">
        ${tag}
        ...
        <span class="sub">${escapeHtml(r.userPseudoId||'')}</span>
      </div>`;
  }).join('');
